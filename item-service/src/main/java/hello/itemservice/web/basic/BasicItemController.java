...
//    @PostMapping("/add")
    public String saveV2(@ModelAttribute("item") Item item, Model model) {
        itemRepository.save(item);
//      model.addAttribute("item", item); //@ModelAttribute("item") item이라는 이름으로 model에 자동 추가
...
//    @PostMapping("/add")
    public String saveV3(@ModelAttribute Item item) {
        itemRepository.save(item);
//      model.addAttribute("item", item); //클래스명의 앞글가 I -> i(item) 로 변경해서 model에 자동 추가
...
//    @PostMapping("/add")
    public String saveV4(Item item) {
        itemRepository.save(item);
//      model.addAttribute("item", item); //V3과 동일 @ModelAttribute가 없어도 동일하게 작동
